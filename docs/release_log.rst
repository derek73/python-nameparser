...
* 2.2.0 - Unreleased
    ...
    **Behavior Changes**
    ...
    - Fix a space-separated credential run after a one-word family comma rendering with a comma the name never had: ``"Smith, MD PhD"`` gives suffix ``MD PhD`` where it gave ``MD, PhD``, and ``"Smith, CBE MC"``, ``"Smith, BSc MBA"`` and ``"Smith, PhD Jr."`` the same. The roles were already right; only the rendered string carried the extra comma. One comma segment is one suffix entry, and grouping decided which segments those were by their position while assignment decided the same segment by its content, so the two disagreed about exactly the segment a one-word family leaves behind. The full-name form was never affected -- ``"John Smith, MD PhD"`` has given suffix ``MD PhD`` since 1.4.0 -- and this brings the one-word form into line with it. A run written with commas keeps them (``"Smith, MD, PhD"`` is still suffix ``MD, PhD``), and a name word in the segment still makes it the given-and-suffix reading (``"Smith, John Jr."``). Round-tripping a parsed name through ``str()`` is stable on these shapes again, the inserted comma having re-segmented on re-parse (closes #429)
