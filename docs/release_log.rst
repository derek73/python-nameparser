...
* 2.4.0 - Unreleased
    ...
    **Behavior Changes**
    ...
    - **The comma paths report an ambiguous reading for the first time.** ``parse("Smith, MA").ambiguities`` names ``suffix-or-name``, and so does every other decision at the ambiguous credential class -- before or after a comma, in either direction, with no new ``AmbiguityKind``. One report per decision: ``Smith, Ma`` reports that the word was kept as the given name just as ``Smith, MA`` reports that it was taken as a credential. One report also goes AWAY: ``Steven Hardman, MD, DO, DDS`` no longer reports ``comma-structure``, its third comma segment now reading as the credential run it always was. See the ``S2`` and ``C1`` entries of ``docs/design/decisions.md``
