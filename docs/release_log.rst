...
* 2.2.0 - Unreleased
    ...
    **Behavior Changes**
    ...
    - Fix a particle chain and a maiden name taking a trailing generational numeral as a name word: ``"John van der Berg V"`` read family ``van der Berg V`` and ``"John née Jones Smith V"`` read maiden ``Jones Smith V``, where ``"John Smith V"`` reads suffix ``V``. Both stops asked "is this a suffix?" with the test that vetoes a bare ``V`` as an initial -- the same question the bound given-name join asked until #401 -- and now stop where the suffix rule's own walk begins: family ``van der Berg``, maiden ``Jones Smith``, suffix ``V``, for ``I`` and ``X`` alike; ``"John van der J. V"`` keeps its reading, the letter before the numeral being an initial. The chain stops before a bare credential with words to spare too, as ``"John Smith Ma"`` does: ``"John van der Berg Ma"`` reads suffix ``Ma``, which is how 1.4.0 read it; a maiden name keeps such a credential, since the words the marker takes are the very words that made it one. No name of the differential corpora has the shapes; the rules examples carry them. The chain also no longer swallows the given name behind an unlisted abbreviation: ``"Xyz. van Johnson"`` reads given ``van``, as ``"Dr. van Johnson"`` has since #367, the chain now asking the same test that reads the abbreviation as a title (closes #424)
