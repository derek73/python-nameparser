...
* 2.0.0 - unreleased
    ...
    **Breaking Changes**
    ...
    - Remove ``Constants.empty_attribute_default`` (#255): empty fields are always ``''``. Assigning it raises ``TypeError``; a pickle carrying the key still loads, with the value ignored
    ...
    - Remove silent unknown-key access on the mapping managers (#256): ``CONSTANTS.capitalization_exceptions.typo`` and ``CONSTANTS.regexes.typo`` raise ``AttributeError`` naming the miss and the known keys, instead of returning ``None``/``EMPTY_REGEX``. ``.get()`` remains available on both for intentional soft access
    ...
    **Behavior Changes**
    ...
    - Fix a lone suffix or title after a comma being routed to ``first``: ``"Andrews, M.D."`` and ``"Smith, Dr."`` now fill ``suffix``/``title``, since the piece before the comma is definitionally the family name
