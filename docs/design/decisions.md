...
- 2026-08-19 #399 (PR #409) — a maiden marker bounds a particle join reaching it from the left. rules.md carried the opposite as an Accepted limitation ("a particle chain swallows a marker in its path, the join running first"), but that was a consequence of stage order rather than a judgment: the chain merges pieces before the marker is consumed, and the consumer looks for a lone marker piece which by then no longer exists. So `Ursula von der Leyen geb. Albrecht` lost the maiden name that `Ursula Leyen geb. Albrecht` kept. Two fixes were priced in-process (mechanisms.md#SIMULATE-EVERY-OPTION): a marker stop in the chain's inner loop, beside the suffix stop it already has, and moving the maiden handler ahead of the chain. The stop won because reordering changes the handler's input from merged pieces to unmerged ones, disturbing names whose MAIDEN side carries its own particle chain ("Anna Müller geb. von der Berg") — which the stop leaves untouched. The stop and the consumer share one definition of a marker piece, so a chain cannot stop where the consumer would not then take. Two things fell out rather than being decided separately: the same issue's open question about the leftover given slot (the marker used to compete for it, giving `de la Cruz née Vega` given "née" under a family-first order; dropped, it never reaches the placement), and the wholly-particle family a stop can leave, which #404's R2 reading already covers — "Jane de la née Jones" reports family "de la" with a non-empty base. Tests are the whole verification: no differential corpus name carries a marker with a particle ahead of it, so a clean gate report says nothing about this class.
