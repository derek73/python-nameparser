...
sorted first, as `main()` loads them); the rule keeps that, on the
...
latter reads `_RECORDED_DIFFS[ledger.name]` and `_WATCHED_DIFFS[ledger.name]` -- so a ledger left out of the
