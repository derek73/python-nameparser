...
- **Every pipeline stage is one module with one public function, named exactly as rules.md and the specs name the stage, and its module docstring declares the contract in three labelled lines**: `Consumes:` what it takes from `ParseState`, `Produces:` what it hands back, `Reads:` which `Policy`/`Lexicon` fields it consults. The `Reads:` line is the load-bearing one — it makes "which stage do I touch for this feature?" a grep rather than a read-through, and it pre-writes what `explain()` would have to say. All nine stages carry it; a module in `_pipeline/` that is NOT a stage says so in its own first line instead (`_assemble.py`: "Not a stage: converts the final ParseState into a public ParsedName", and it omits `Reads:` because it consults neither Policy nor Lexicon), so absence of the three lines is a claim about the module, not an oversight. Recorded here 2026-08-16: this was §5 of the 2026-07-11 conventions spec and the only section that never reached a committed home — the spec's own header attests to distilling "§1–§4, §6–§9", which reads as exhaustive unless you notice §5 missing from the middle of the range.
